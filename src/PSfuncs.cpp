...
List pair_search7q(NumericVector x, IntegerVector sorted_indexes) {
  ...
  while(i<(N)){
    ...
    if((j-i)>1){
      ...
      int temp=findindexitq(sorted_indexes[Range(i,j-1)],n);

      if(temp>=n && sorted_indexes[i]!=temp ){
        m=binarySearchIndexq(sorted_indexes[Range(i,j-1)],temp);
...
List pair_search4q(NumericVector x, NumericVector y) {
  ...
  //initiate list
  List list(0);
  IntegerVector sorted_indexes=sorted_index_vectorq(x);//(index_vec.begin(), index_vec.end());
  std::sort(x.begin(),x.end());
  int i=0;
  while(i<N){
    //initialize m to get split index
...
IntegerMatrix pair_search8q(NumericVector x, NumericVector y) {
  ...
  while(i<N){
    ...
    if(((j-i)>1)&&m>0){
      m1=expandGridq((sorted_indexes[Rcpp::Range(i,m-1)]+1),((sorted_indexes[Rcpp::Range(m,j-1)])-n+1));//indexes from second vector
